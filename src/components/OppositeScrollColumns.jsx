...
import { ScrollContainerStyle, AboutColumnsContent, AboutSections } from "../styles/about-style";
import { BtnAbout } from "../styles/buttons";
const MemoAboutSections = React.memo(({ children }) => <AboutSections>{children}</AboutSections>);

const OppositeScrollColumns = ({ arrLeft, arrRight }) => {
  const { containerRef, scrollY, handleArrowUp, handleArrowDown, SECTION_HEIGHT, TOTAL_SECTIONS } = useOppositeScroll();
  const { handleCursorXs, handleCursorSmall } = useCustomCursorContext();
  const currentSection = Math.round(scrollY / SECTION_HEIGHT);

  return (
    <ScrollContainerStyle ref={containerRef}>
      {currentSection > 0 && (
        <BtnAbout
      ...
      >
        {arrLeft.map((child, index) => (
          <MemoAboutSections key={index}>{child}</MemoAboutSections>
        ))}
      </AboutColumnsContent>
      <AboutColumnsContent
        onMouseLeave={handleCursorSmall}
